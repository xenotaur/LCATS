...
### N-way genre-deviation prototype

`figures/nway_genre_deviation/` is generated by
`generators/make_worldcon_nway_genre_deviation.py`. It extends the preserved
pairwise figures into one reusable reference-to-many result and renderer:

- the left panel shows per-million frequency in the full 146-story balanced
  sample, pointing left;
- the word column fixes a common aligned vocabulary (the reference's top 20);
- all eight selection-genre panels show `genre - reference` on one joint,
  symmetric deviation scale;
- negative and positive deviations use direction, hatching, and red/blue color;
- `--highlight off|per-genre|global` controls whether extrema are darkened,
  including ties and omitting a sign when no value has that sign.

The CSV is long-form (one term/panel cell per row), and the manifest records
the exact universe, selectors, denominators, deviation direction, shared scale,
and highlighted cells. The figure is descriptive, not a significance test.

For a similar datasource, construct a `comparison.ComparisonCorpus` with one
document per story, populate the appropriate membership fields, then reuse
`comparison.compare_many` and `rendering.plot_nway_deviation_comparison`.

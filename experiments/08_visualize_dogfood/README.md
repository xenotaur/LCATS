...
### N-way genre-deviation examples

`figures/nway_genre_deviation/` is generated by
`generators/make_worldcon_nway_genre_deviation.py`, a thin wrapper that only
chooses inputs, genres, policies, and output names. Analysis, rendering, CSV,
and manifest writing are the supported `lcats.visualize` facility
(`comparison.compare_many`, `rendering.NWayRenderSpec`, and
`nway_outputs.write_nway_outputs`), also exposed as
`lcats visualize compare-many`. All examples use the 146-story balanced sample
as `U` and per-million frequency with the reference's (or, without a common
reference, the largest deviations') top 20 terms:

| Stem | Panels | Reference | Presentation |
|---|---|---|---|
...
order, metric, and symmetric deviation scale. Each CSV is long-form (one
term/panel cell per row) with values, per-cell references, signed deviations,
denominators, axis limits, and band/column positions; each manifest records
selectors and resolved memberships, all pairwise overlaps, complement
construction, the render spec and layout, and SHA-256 hashes of the CSV and
every figure. The eight selection genres happen to be disjoint and
exhaustive in this sample, which the manifest reports as observed facts
(`pairwise_disjoint`, `covers_universe`) while keeping `partition_claim:
false`. The figures are descriptive, not significance tests.

The earlier `lcats_146_nway_genre_deviation_per_genre.*` files are the
preserved PR #442 prototype (schema `lcats-nway-comparison-v1`, left-pointing
reference with a central word column). The generator no longer writes that
stem, so they are not overwritten.

For a similar datasource, construct a `comparison.ComparisonCorpus` with one
document per story, populate the appropriate membership fields, build an
`NWayComparisonSpec`, and call `nway_outputs.write_nway_outputs`.
Resolver-specific POS/lemma adaptation belongs before that boundary: normalize
the resolved token evidence into each document's comparison text (or a future
lexical-evidence adapter) without teaching the renderer about the resolver.
...
python experiments/08_visualize_dogfood/generators/make_worldcon_nway_genre_deviation.py
```

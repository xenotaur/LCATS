...
def find_paragraphs_fourmillion(soup, start_heading_text):
    ...
    #        lambda tag: print(str(tag.name) + "----" + tag.get_text(strip=True) + " ----> " + start_heading_text + ":" + str(start_heading_text in tag.get_text(strip=True))))

 #       tag.name in ('h2', 'h3') and start_heading_text in tag.get_text(strip=True))

...
GRIMM_HEADINGS = [
    ...
    ('travelling_musicians', 'THE TRAVELLING MUSICIANS', 'Grimm - the Travelling Musicians'),
    ('old_sultan', 'OLD SULTAN', 'Grimm - Old Sultan'),
    ('straw,_the_coal,_and_the_bean', 'THE STRAW, THE COAL, AND THE BEAN', 'Grimm - the Straw, the Coal, And the Bean'),
    ('briar_rose', 'BRIAR ROSE', 'Grimm - Briar Rose'),
    ('dog_and_the_sparrow', 'THE DOG AND THE SPARROW', 'Grimm - the Dog And the Sparrow'),
    ('twelve_dancing_princesses', 'THE TWELVE DANCING PRINCESSES', 'Grimm - the Twelve Dancing Princesses'),
    ('fisherman_and_his_wife', 'THE FISHERMAN AND HIS WIFE', 'Grimm - the Fisherman And His Wife'),
    ('willow-wren_and_the_bear', 'THE WILLOW-WREN AND THE BEAR', 'Grimm - the Willow-wren And the Bear'),
    ('frog-prince', 'THE FROG-PRINCE', 'Grimm - the Frog-prince'),
    ('cat_and_mouse_in_partnership', 'CAT AND MOUSE IN PARTNERSHIP', 'Grimm - Cat And Mouse In Partnership'),
    ('goose-girl', 'THE GOOSE-GIRL', 'Grimm - the Goose-girl'),
    ('adventures_of_chanticleer_and_partlet', 'THE ADVENTURES OF CHANTICLEER AND PARTLET', 'Grimm - the Adventures Of Chanticleer And Partlet'),
    ('rapunzel', 'RAPUNZEL', 'Grimm - Rapunzel'),
    ('fundevogel', 'FUNDEVOGEL', 'Grimm - Fundevogel'),
    ('valiant_little_tailor', 'THE VALIANT LITTLE TAILOR', 'Grimm - the Valiant Little Tailor'),
    ('hansel_and_gretel', 'HANSEL AND GRETEL', 'Grimm - Hansel And Gretel'),
    ('mouse,_the_bird,_and_the_sausage', 'THE MOUSE, THE BIRD, AND THE SAUSAGE', 'Grimm - the Mouse, the Bird, And the Sausage'),
    ('mother_holle', 'MOTHER HOLLE', 'Grimm - Mother Holle'),
    ('little_red-cap_[little_red_riding_hood]', 'LITTLE RED-CAP [LITTLE RED RIDING HOOD]', 'Grimm - Little Red-cap [little Red Riding Hood]'),
    ...
    ('old_man_and_his_grandson', 'THE OLD MAN AND HIS GRANDSON', 'Grimm - the Old Man And His Grandson'),
    ('little_peasant', 'THE LITTLE PEASANT', 'Grimm - the Little Peasant'),
    ('frederick_and_catherine', 'FREDERICK AND CATHERINE', 'Grimm - Frederick And Catherine'),
    ...
    ('wolf_and_the_seven_little_kids', 'THE WOLF AND THE SEVEN LITTLE KIDS', 'Grimm - the Wolf And the Seven Little Kids'),
    ('queen_bee', 'THE QUEEN BEE', 'Grimm - the Queen Bee'),
    ('elves_and_the_shoemaker', 'THE ELVES AND THE SHOEMAKER', 'Grimm - the Elves And the Shoemaker'),
    ...
    ('four_clever_brothers', 'THE FOUR CLEVER BROTHERS', 'Grimm - the Four Clever Brothers'),
    ...
    ('king_of_the_golden_mountain', 'THE KING OF THE GOLDEN MOUNTAIN', 'Grimm - the King Of the Golden Mountain'),
    ('doctor_knowall', 'DOCTOR KNOWALL', 'Grimm - Doctor Knowall'),
    ('seven_ravens', 'THE SEVEN RAVENS', 'Grimm - the Seven Ravens'),
    ('wedding_of_mrs_fox', 'THE WEDDING OF MRS FOX', 'Grimm - the Wedding Of Mrs Fox'),
    ('salad', 'THE SALAD', 'Grimm - the Salad'),
    ('story_of_the_youth_who_went_forth_to_learn_what_fear_was', 'THE STORY OF THE YOUTH WHO WENT FORTH TO LEARN WHAT FEAR WAS', 'Grimm - the Story Of the Youth Who Went Forth To Learn What Fear Was'),
    ...
    ('snow-white_and_rose-red', 'SNOW-WHITE AND ROSE-RED', 'Grimm - Snow-white And Rose-red')
]
...
def find_paragraphs_grimmfairytales(soup, start_heading_text):
    ...
    while current_element and current_element.name not in ('h2', 'div'):
        if current_element.name == 'p' or current_element.name == 'pre':
            paragraphs.append(re.sub(r'[\xc2-\xf4][\x80-\xbf]+',lambda m: m.group(0).encode('latin1').decode('utf8'),current_element.get_text(strip=False)))
...
def create_download_callback(story_name, url, start_heading_text, description):
    ...
    def story_download_callback(contents):
        ...
        story_text = find_paragraphs_grimmfairytales(story_soup, start_heading_text)

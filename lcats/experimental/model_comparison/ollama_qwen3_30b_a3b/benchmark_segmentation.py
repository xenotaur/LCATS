"""Local-model candidate: qwen3:30b-a3b against the scene/sequel
segmentation stage.

WI-LLM-0051 investigation variant - checks whether the `tool_choice`
gap `../ollama_qwen3_8b/benchmark_segmentation.py` reproduces is
model-specific or general, by holding the story fixed and varying only
the model. Run setup.py first - it confirms Ollama is running and the
model is pulled. See ../common/harness.py's run_segmentation() for the
real call path this reuses.

Usage:
    python lcats/experimental/model_comparison/ollama_qwen3_30b_a3b/benchmark_segmentation.py
"""
...
def main() -> None:
    ...
    result = harness.run_segmentation(
        ...
        # across models, not re-validating the retry mitigation (already
        # validated on qwen3:8b) - disable it so a retry success here
        # doesn't get conflated with the baseline-frequency evidence.

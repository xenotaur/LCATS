...
DEV_CORPUS = os.path.abspath(
    os.path.join(PROJECT_ROOT, "data")
)  # Local copy of the data.
DEV_OUTPUT = os.path.abspath(
    os.path.join(PROJECT_ROOT, "output")
)  # Local copy of the data.
GIT_CORPUS = os.path.abspath(
    os.path.join(PROJECT_ROOT, "../corpora")
)  # Data in the git repo.
OPENIA_API_KEYS_ENV = os.path.abspath(
    os.path.join(PROJECT_ROOT, "../.secrets/openai_api_keys.env")
...
def remind_me_with_troika(situation, number=1, model_name="gpt-3.5-turbo"):
    ...
    memory_scene = gpt_4o_data[story_name]["extracted_output"][memory_id_event_number]
    ...
    solution = adapt_with_llm(situation, memory_scene_parts)

    if verbose:
        print("An adapted solution is to --> " + solution)
...
def extract_all_and_write(
    corpora, extractor, model_name, output_dir, file_namer, serializer
):
...
def fetch_story_extraction(story, model_name, output_dir, file_namer):
    # Determine file path
    model_dir = os.path.join(output_dir, "scene_extraction", model_name)
    os.makedirs(model_dir, exist_ok=True)
    filename = file_namer(story.name) + "-scenes.json"
...
def fetch_or_compute_story_extraction(
    story, extractor, model_name, output_dir, file_namer, serializer, force=False
):
    """
...
def fetch_or_compute_story_extras(
    story, extraction, model_name, output_dir, file_namer, serializer, force=False
):
    """
    Fetch the scene/sequel extraction for a story, loading from disk if available,
    or computing and saving it otherwise.
...
def make_scene_parts_query(situation: str, model_name="gpt-3.5-turbo") -> Dict:
    ...
    message_to_send = [
        {"role": "user", "content": "'" + messages + " " + str(situation) + " "}
    ]
...
def make_scene_parts(extracted_scene: str, model_name="gpt-3.5-turbo") -> Dict:
    ...
    message_to_send = [
        {"role": "user", "content": "'" + messages + " " + str(extracted_scene) + " "}
    ]  # list(extracted_scene)[0] + ' ' + list(extracted_scene)[1] + "'"}]
    ...
    message_to_send = [
        {
            ...
            + messages
            + " "
            + list(extracted_scene)[0]
            + " "
            + list(extracted_scene)[1]
            + "'",
        }
    ]
...
def adapt_with_llm(new_situation, old_scene, model_name="gpt-3.5-turbo"):
    messages = (
        "Take the following new situation: '"
        + new_situation
        + "' and try to adapt it using only the information in the following structure: "
        + str(old_scene)
        + "'.  If the goal of the structure is similar to the goal in the new situation, adapt the action from the structure to match the new situation including changing the character from the structure to match the new situation.   Suggest ways to avoid the catastrophe in the structure adapted to the new situation.  Present the answer in paragraph form, not as a JSON structure."
    )
...
def try_it(new_sit=None):
    global names1, cl1, ex1, scenes, st1, indexes, choice

    if new_sit == None:
        print("Setting up a test bench")
        choice = random.randint(0, 4)
        names1, cl1 = build_test_set(5)
        ex1 = cl1[choice]["extracted_output"]
        scenes = get_scenes(ex1)
        st1 = scenes[1]
        indexes = build_scenes(st1)

        print(st1)
    else:
        print("Running with " + names1[choice])
        return adapt_with_llm(new_sit, indexes)


def build_scenes(extracted_scene: str, model_name="gpt-3.5-turbo") -> Dict:
    messages = "Take the following extracted scene which is made up of text and reason and produce a JSON structure consisting of 'goal', 'action', and 'catastrophe."
...
def play_indexes(extracted_scene, model_name="gpt-3.5-turbo"):
    ...
    message_to_send = [
        ...
        {"role": "user", "content": str(scene_parts)},
    ]
...
def make_indexes_for_scene(extracted_three_part_scene, model_name="gpt-3.5-turbo"):
    message_to_send = [
        ...
        {"role": "user", "content": str(extracted_three_part_scene)},
    ]
...
def best_match_cosine(keywordsTarget, memory, number):
    ...
    for i, keywords in enumerate(keywordsArray):
        ...
        vectorizer = CountVectorizer().fit_transform(
            [keywordsTargetString, keywordsMatchString]
        )
...
def test_build_memory(number):
    names, story_data = build_test_set(number)

    return build_memory(names, story_data)
...
def build_all_scenes_indexes(
    extracted_story: str, story_name, model_name="gpt-3.5-turbo"
) -> Dict:
    all_scenes = []
...
def KMMbuild_all_indexes(
    extracted_story_text: str, model_name="gpt-3.5-turbo", general=True
) -> Dict:
    if general:
...
def build_test_set(number=10):
    ...
    for i in range(0, number):
        ...
        stories.append(gpt_4o_data[corpora.stories[index].name])
...
def index_one_and_write(
    ...
    # Create a subdirectory for the model
    model_dir = os.path.join(output_dir, "extraction-extras", model_name)
    os.makedirs(model_dir, exist_ok=True)
...
for story in corpora.stories:
    ...
    print(
        f" - events: {len(gpt_4o_events)} (gpt-4o) vs {len(gpt_35_events)} (gpt-3.5-turbo)"
    )
    print(
        f" - scenes: {len(gpt_4o_scenes)} (gpt-4o) vs {len(gpt_35_scenes)} (gpt-3.5-turbo)"
    )
    print(
        f" - sequels: {len(gpt_4o_sequels)} (gpt-4o) vs {len(gpt_35_sequels)} (gpt-3.5-turbo)"
    )
    print(
        f" - nones: {len(gpt_4o_nones)} (gpt-4o) vs {len(gpt_35_nones)} (gpt-3.5-turbo)"
    )
...
def plot_columns_vs(
    df, x_col, y_col, y_low=0, y_high=300, log=False, hue_col=None, legend=""
):

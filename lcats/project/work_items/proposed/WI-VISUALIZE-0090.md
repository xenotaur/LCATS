...
acceptance:
  ...
  - "The **default** (no `--contrast`) behavior and output schema are completely unchanged -- every existing `tfidf_top_terms`/`run_tfidf` test continues to pass unmodified, confirming this is additive, not a breaking change. The existing within-group-mean-salience mode (and its own accurate documentation, delivered by `WI-VISUALIZE-0089`) remains available and correct"

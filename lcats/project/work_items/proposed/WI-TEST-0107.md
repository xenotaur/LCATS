...
acceptance:
  ...
  - In-scope noisy tests suppress output at their individual call sites using lcats.utils.capture, while tests that assert output continue using capture_output.
...
- Suppress output locally with `capture.suppress_output()` or capture it with `capture.capture_output()` when assertions require inspection.
...
5. Avoid modifying `scripts/test` to suppress output globally.
6. Run the full validation sequence and record the before/after output behavior.

...
acceptance:
  ...
  - "A new, shared sidecar-validator registry module exists in analysis/corpus/, mapping registered sidecar filenames to validator callables, registering all 4 currently-produced sidecar kinds (genre.json, scenes.json, linguistics.json, linguistics.tokens.json); promote.py's insert/upsert payload-validation dispatch imports only this registry, never genre_sidecar.py or linguistics/sidecar.py directly. Two direct genre_sidecar usages predate this work item and are explicitly out of scope: insert/upsert's own destination-overwrite guard in _promote_sidecar_records (checking for an existing legacy-flat genre.json before writing) is a separate concern from payload-validation dispatch and retains a direct genre_sidecar shape-detection call; replace mode's own structural pre-flight check (_validate_sidecars) separately retains a direct genre_sidecar import for shape-detection and current-format validation of source content. See WI-PROMOTE-0102 (amended per WI-PROMOTE-0102's investigation, not part of this WI's original resolution)"

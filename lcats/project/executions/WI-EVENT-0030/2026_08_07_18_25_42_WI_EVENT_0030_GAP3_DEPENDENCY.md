...
- **Missing dependency, confirmed and fixed:** `WI-EVENT-0030`'s `depends_on` listed `WI-EVENT-0029` and `WI-ASSESS-0031` but not `WI-ASSESS-0051` (Gap 2's own work item), even though the design doc's Gap 3 description explicitly says re-scoping "depends on both `WI-ASSESS-0031` and the corpus survey" (Gap 2). Added `WI-ASSESS-0051` to `depends_on`.
